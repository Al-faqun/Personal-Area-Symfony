...
function registerCargo() {
    ...
    var params = 'container=' + encodeURIComponent(container) + '&date_arrival=' + encodeURIComponent(date_arrival);
    xhttp.open("POST", Routing.generate('new_cargo'));
...
function makeExecutor(cargoID) {
    $.ajax({
        type: 'post',
        url: Routing.generate('set_manager'),
